...
<?php include("includes/header.php"); ?>
	...
	<?php
$connection = @mysqli_connect('localhost', root, '', namelist) or die("Нет соединения с БД");
...
if(!empty($_POST['fullname']) && !empty($_POST['email']) && !empty($_POST['username']) && !empty($_POST['password'])) {
	...
	$query=mysqli_query($connection,"SELECT * FROM namelist WHERE username='".$username."'");
	$numrows=mysqli_num_rows($query);
	
	if($numrows==0)
	{
	$sql="INSERT INTO namelist
